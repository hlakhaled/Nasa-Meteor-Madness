...
const ImpactAnalysis = ({ target, asteroid, onClose }) => {
  ...
  const isOcean = target.region === "Ocean" || target.region === "Arctic" || target.region === "Antarctica";
    ...
    if (diameter < 50) {
      if (isOcean) {
        return [
          ...
          `Crater: ${(craterDiameter / 1000).toFixed(1)} km diameter (mostly underwater)`,
          `Steam explosion creating localized waves up to ${(diameter * 0.5).toFixed(0)}m high`,
          `Shockwave affecting marine life within ${(diameter * 2).toFixed(0)} km`,
          `Magnitude ${magnitude.toFixed(1)} seismic event detectable on nearby coasts`,
          `Thermal radiation heating water surface up to ${thermalRadius.toFixed(1)} km away`,
        ...
        return [
          ...
          `Crater: ${(craterDiameter / 1000).toFixed(2)} km diameter, ${(craterDiameter / 1000 / 5).toFixed(2)} km deep`,
          `Fireball radius: ${fireballRadius.toFixed(1)} km - immediate vaporization`,
          `Air blast destroying buildings: ${airBlastRadius.toFixed(1)} km radius`,
          `Thermal radiation (3rd degree burns): ${thermalRadius.toFixed(1)} km`,
          `Magnitude ${magnitude.toFixed(1)} earthquake`,
          `Ejecta pattern covering ${(craterDiameter * 3 / 1000).toFixed(0)} km²`,
      ...
      if (isOcean) {
        ...
        return [
          ...
          `Underwater crater: ${(craterDiameter / 1000).toFixed(1)} km diameter`,
          `Initial wave height: ${tsunamiHeight.toFixed(0)}m at point of impact`,
          `Tsunami affecting coastlines up to ${tsunamiRange.toFixed(0)} km away`,
          `Wave arrival time: ${(tsunamiRange / 800).toFixed(1)} hours to distant coasts`,
          `Magnitude ${magnitude.toFixed(1)} underwater earthquake`,
          `Massive steam column reaching stratosphere (${(diameter * 0.4).toFixed(0)} km high)`,
          `Acid rain from vaporized seawater affecting ${(diameter * 10).toFixed(0)} km radius`,
          `Marine ecosystem devastation in impact region`,
          `Global ocean temperature increase: ${(energy / 1000000).toFixed(3)}°C`,
          `Estimated coastal casualties: millions if densely populated areas affected`
        ];
      } else {
        return [
          `🏔️ Regional land impact at ${target.region}`,
          `Crater: ${(craterDiameter / 1000).toFixed(1)} km diameter, ${(craterDiameter / 1000 / 5).toFixed(1)} km deep`,
          `Fireball radius: ${fireballRadius.toFixed(1)} km - complete vaporization`,
          `Air blast (5 psi): ${airBlastRadius.toFixed(0)} km - total structural collapse`,
          `Thermal radiation: ${thermalRadius.toFixed(0)} km radius (3rd degree burns)`,
          `Magnitude ${magnitude.toFixed(1)} earthquake felt across ${diameter < 500 ? "region" : "continent"}`,
          `Ejecta blanket: molten/fragmented material covering ${(craterDiameter * 4 / 1000).toFixed(0)} km radius`,
          `Firestorms igniting within ${(thermalRadius * 0.7).toFixed(0)} km`,
          `Atmospheric dust blocking sunlight regionally for weeks`,
          `Temperature drop: ${(1 + energy / 5000).toFixed(1)}°C for ${diameter < 500 ? "months" : "1-2 years"}`,
      ...
      if (isOcean) {
        ...
        return [
          ...
          `Massive crater: ${(craterDiameter / 1000).toFixed(0)} km diameter underwater`,
          `Mega-tsunami: ${tsunamiHeight.toFixed(0)}m waves devastating ALL coastlines`,
          `Wave speed: 800 km/h, reaching distant continents within hours`,
          `Entire ocean basin affected - waves rebound for days`,
          `Magnitude ${magnitude.toFixed(1)} earthquake - one of strongest ever recorded`,
          ...
          `Temporary ocean temperature increase: ${(energy / 500000).toFixed(1)}°C`,
          `Coastal cities worldwide completely destroyed`,
          `Global climate disruption: ${(5 + energy / 10000).toFixed(0)}°C cooling for ${diameter < 2000 ? "2-4" : "4-8"} years`,
        ...
        return [
          ...
          `Impact crater: ${(craterDiameter / 1000).toFixed(0)} km diameter, visible from orbit`,
          `Fireball: ${fireballRadius.toFixed(0)} km - vaporizes everything to bedrock`,
          `Air blast: ${airBlastRadius.toFixed(0)} km - flattens all structures`,
          `Magnitude ${magnitude.toFixed(1)} earthquake - strongest geological event in human history`,
          `Ejecta ejected into space, re-entering globally as molten rain`,
          `Continental firestorms igniting ${diameter < 2000 ? "40" : "70"}% of nearby forests`,
          `Shock wave circling Earth multiple times`,
          `Dust veil blocking ${diameter < 2000 ? "60" : "80"}% of sunlight for 3-6 years`,
          `Global temperature drop: ${(10 + energy / 15000).toFixed(0)}°C (Impact Winter)`,
      ...
      if (isOcean) {
        return [
          ...
          `Impact crater: ${(craterDiameter / 1000).toFixed(0)} km - penetrates ocean floor to mantle`,
          `Mega-tsunamis: 300-2000m waves submerging all low-lying continents`,
          `Ocean water vaporized: ${(energy / 10000).toFixed(0)} cubic kilometers`,
          `Magnitude ${magnitude.toFixed(1)} mega-quake - triggers global volcanism`,
          `Shockwave boiling ocean surface across ${(fireballRadius * 2).toFixed(0)} km`,
        ...
        return [
          ...
          `Crater: ${(craterDiameter / 1000).toFixed(0)} km diameter - reshapes continental geology`,
          `Mantle exposed - molten rock ejected globally`,
          `Fireball encompassing ${fireballRadius.toFixed(0)} km - surface temperature 5000°C`,
        ...
        <div style={{ padding: "clamp(10px, 2vw, 14px)" }}>
          <div style={{ 
            ...
            <div style={{ 
              ...
              <div className="inter-text" style={{ fontSize: "clamp(9px, 1vw, 10px)", color: "#999", marginBottom: "4px" }}>Diameter</div>
              <div className="inter-text" style={{ fontSize: "clamp(13px, 1.8vw, 16px)", fontWeight: "700", color: devastation.border }}>
                {asteroid.diameter >= 1000 ? `${(asteroid.diameter/1000).toFixed(1)} km` : `${asteroid.diameter}m`}
              </div>
            </div>
            <div style={{ backgroundColor: "#141416", padding: "clamp(8px, 1.2vw, 10px)", borderRadius: "8px", border: "1px solid #2a2a2d" }}>
              <div className="inter-text" style={{ fontSize: "clamp(9px, 1vw, 10px)", color: "#999", marginBottom: "4px" }}>Velocity</div>
              <div className="inter-text" style={{ fontSize: "clamp(13px, 1.8vw, 16px)", fontWeight: "700", color: devastation.border }}>{asteroid.velocity} km/s</div>
            </div>
            <div style={{ backgroundColor: "#141416", padding: "clamp(8px, 1.2vw, 10px)", borderRadius: "8px", border: "1px solid #2a2a2d" }}>
              <div className="inter-text" style={{ fontSize: "clamp(9px, 1vw, 10px)", color: "#999", marginBottom: "4px" }}>Mass</div>
              <div className="inter-text" style={{ fontSize: "clamp(13px, 1.8vw, 16px)", fontWeight: "700", color: devastation.border }}>
                {parseFloat(asteroid.mass) >= 1000 ? `${(parseFloat(asteroid.mass)/1000).toFixed(1)}k tons` : `${asteroid.mass} tons`}
              </div>
            </div>
            <div style={{ backgroundColor: "#141416", padding: "clamp(8px, 1.2vw, 10px)", borderRadius: "8px", border: "1px solid #2a2a2d" }}>
              <div className="inter-text" style={{ fontSize: "clamp(9px, 1vw, 10px)", color: "#999", marginBottom: "4px" }}>Impact Energy</div>
              <div className="inter-text" style={{ fontSize: "clamp(13px, 1.8vw, 16px)", fontWeight: "700", color: devastation.border }}>{asteroid.energy}</div>
            </div>
          </div>
          ...
          <div style={{ 
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "clamp(7px, 1.2vh, 9px)", position: "relative" }}>
              {effects.map((effect, index) => (
                ...
                onMouseLeave={(e) => {
                  e.currentTarget.style.backgroundColor = index % 2 === 0 ? "rgba(0,0,0,0.2)" : "transparent";
                  e.currentTarget.style.transform = "translateX(0)";
                }}
                >
                  <span style={{ fontSize: "clamp(11px, 1.4vw, 13px)", flexShrink: 0, filter: "grayscale(0%)" }}>
                  ...
                  <span className="inter-text" style={{ 
                    fontSize: "clamp(10px, 1.2vw, 12px)", 
                    color: "#fff", 
                    lineHeight: "1.5",
                    fontWeight: effect.includes("extinction") || effect.includes("civilization") || effect.includes("casualties") ? "600" : "400"
                  }}>
                    {effect.replace(/^(🌊|🏔️)\s/, '')}
                  </span>
                </div>
